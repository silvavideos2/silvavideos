...
case 'oi':
tujuh = fs.readFileSync('./audios/ola.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break

const imageh = fs.readFileSync('./fotobot/foto.jpeg')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'espera um pouco ai cunhadokkj',
				success: 'pronto cunhado',
				error: {
					stick: 'Erro kkkkj',
					Iv: 'LINK INVALIDO',
				},
				only: {
					group: '❌ SO PODE SER USADO EM GRUPO MEU CHAPA❌',
					ownerG: 'ISSO E COMANDO PARA O ADEMIR KKK❌',
					ownerB: '❌ ESSE COMANDO SO PODE SER USADO PELO SILVA❌',
					admin: 'ESSE COMANDO SO PODE SER USADO PELO SILVA ❌',
					Badmin: 'ME COLOCA DE ADM AI pow',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["556199730280@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'ytsearch':
					if (args.length < 1) return reply('TA PROCURANDO OQ MEU CUNHADO? ')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'CADE O NOME DE USUARIO?', text, {quoted: mek})
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('ESSE NOME É INVALIDO K')
					}
					break
			
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('OQ TU QUER ESCREVER?')
					teks = body.slice(7)
					reply(mess.wait)
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					...
					reply('TODOS O CHATES FORAM APAGADOS KKK')
					break
				
				case 'bc':
					if (!isOwner) return reply('QUEM É A PESSOA ZÉ?') 
					...
					if (mentioned.length > 1) {
						...
						mentions(`PROMOVEU @${mentioned[0].split('@')[0]} A REI DOS GAY`, mentioned, true)
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'AGORA ELE NAO É MAIS ADM,CHUPA O CU DELE KKKK\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						...
						mentions(`REBAIXOU @${mentioned[0].split('@')[0]} SOLDADO DESPROMOVIDO`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('QUEM TU QUER ADD?') 
					if (args[0].startsWith('08')) return reply('ESCREVE DIREITO POW KKK')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('NAO FOI POSSIVEL ADICIONAR' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('MARQUE O FDP!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'FOI TARDE FDP:\n'
						for (let _ of mentioned) {
						...
						mentions(`SAIU PRA DAR O CU : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
case 'silva':
tujuh = fs.readFileSync('./audios/silva.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break

case 'adm':
tujuh = fs.readFileSync('./audios/adm.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
...
case 'canta':
tujuh = fs.readFileSync('./audios/canta.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break

case 'kkk':
tujuh = fs.readFileSync('./audios/kkk.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
breakargs.length < 1) return reply('Tag target yang ingin di clone')
